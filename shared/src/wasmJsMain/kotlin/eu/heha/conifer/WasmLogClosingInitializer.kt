 ...
 *
 * So here too the marker means "the run was put away properly", and a log without it belongs to a
 ...
 */
object WasmLogClosingInitializer : LogClosingInitializer {
    override fun installHandler(closeLog: () -> Unit, reopenLog: () -> Unit) {
        ...
        // `pageshow` also fires on the ordinary first load, where there is nothing to reopen and a
        // "reopened" line above the log's own first line would read oddly.
        window.addEventListener("pageshow", { _: Event -> if (hasBeenClosed) reopenLog() })
    }
}
